 ...
 *
 * [operatorId] 는 **값이 항상 하나여도 둔다** (PLAN §11.3). 스테이션에 소유자 개념이 없으면
 * 나중에 로밍(OCPI)을 붙일 때 전 데이터 마이그레이션이 필요해진다. 지금은 설정에서 온
 * 상수 하나지만, 자리가 있다는 것이 요점이다.
 *
 * 스테이션이 보낸 필드는 **모르는 것까지 버리지 않고** 담는다 (PLAN §11.0). `serialNumber` 는
 * 나중에 배터리 이력 대조에, `firmwareVersion` 은 장애 상관분석에 쓰인다.
 *
 * @param authMethod 이 등록이 **어떻게 확인된 신원** 위에서 이뤄졌는지 (PLAN §11.4).
 ...
 * 부팅한 스테이션들 (인메모리).
 *
 * 영속은 하지 않는다 — 원문은 이미 `OcppEventLog` 에 남아 있고 (PLAN §11.1), 이 표는 거기서
 * 재구성 가능한 파생 상태다. 그게 §11.1 이 요구하는 유일한 규칙이다.
 *
 * 스레드 안전하다.
 */
@Component
