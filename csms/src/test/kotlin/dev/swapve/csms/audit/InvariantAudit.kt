...
/**
 * ★★ **부하 뒤의 불변식 감사** (성공 기준 S4, 게이트 L3).
 *
 * > *"스테이션 20대 동시 접속 후 불변식 감사 전항목 통과 — 배터리 수량 보존, 슬롯 이중 예약 0,
 * > 유실 메시지 0"*
 *
 * ### 감사는 이벤트 로그에서 재구성해 검증한다
 *
 * 이벤트 로그의 규칙은 *"파생 상태는 이 로그에서 계산될 수 있어야 한다"* 를 유일한 규칙으로 둔다.
...
class InvariantAudit(
    ...
    /**
     * **완료된 교환마다 들어온 수 = 나간 수** (`COMPLETED` 불변식).
     *
    ...
    private fun batteryConservation(): AuditItem {
        ...
        return AuditItem("배터리 수량 보존", "불변식", checked, "건 교환", violations)
    }

    // ------------------------------------------------------------------ 2. 슬롯 이중 예약 (불변식)

    /**
     * **한 슬롯이 동시에 두 교환에 묶이지 않는다**.
     *
     * 교환 하나가 슬롯을 쥐고 있는 구간을 `[첫 사건 seq, 마지막 사건 seq]` 로 잡고, 같은
     * 스테이션의 같은 슬롯에서 두 구간이 겹치는지 본다. `seq` 는 **스테이션 안에서만** 순서를
     * 보장하므로 비교도 스테이션 안에서만 한다 — 스테이션을 가로지르는 전역
     * 순서는 애초에 없는 약속이다.
     *
     * 한 교환 안에서 입고 슬롯과 출고 슬롯이 겹치는 경우도 이중 예약이다. `TC_S_103_CSMS` 가
     * 입고 A·B 와 출고 C·D 를 **다른 슬롯**으로 지정하는 이유가 그것이다.
     */
    private fun slotDoubleBooking(): AuditItem {
        ...
        return AuditItem("슬롯 이중 예약 0", "불변식", checked, "개 슬롯점유", violations)
    }
    ...
    private fun lostMessages(): AuditItem {
        ...
        return AuditItem("유실 메시지 0", "불변식", checked, "건 CALL", violations)
    }

    // ------------------------------------------------------------------ 4. (stationId, requestId) 유일 (불변식)
    ...
    private fun swapKeyUniqueness(): AuditItem {
        ...
        return AuditItem("(stationId, requestId) 유일", "불변식", checked, "개 상관키", violations)
    }

    // ------------------------------------------------------------------ 5. 교환/충전 분리 (생명주기 분리)

    /**
     * **충전 트랜잭션이 교환 완료로 인해 끊기지 않았다**.
     *
    ...
    private fun swapChargingSeparation(): AuditItem {
        ...
        return AuditItem("교환/충전 분리", "생명주기 분리", checked, "개 슬롯", violations)
    }

    // ------------------------------------------------------------------ 6. 이벤트 로그 순서 (이벤트 로그)

    /**
     * **스테이션별 `seq` 가 빠짐없이 증가한다**.
     *
     * 결번이 있으면 그 사이에 무슨 일이 있었는지 알 수 없고, 리플레이로 상태를 재구성한다는
     * 약속(이벤트 로그 표)이 무너진다. 20 대가 동시에 쓰는 로그이므로 이 항목이 곧 **순번 부여의
    ...
    private fun eventLogOrdering(): AuditItem {
        ...
        return AuditItem("이벤트 로그 순서", "이벤트 로그", checked, "대 스테이션", violations)
    }

    // ------------------------------------------------------------------ 7. 재구성 ↔ 레지스트리 (이벤트 로그)

    /**
     * ★ **로그에서 되살린 상태가 인메모리 레지스트리와 일치한다**.
     *
     * 이벤트 로그의 유일한 규칙이 *"파생 상태는 이 로그에서 계산될 수 있어야 한다"* 이므로, 이
    ...
    private fun replayMatchesRegistry(): AuditItem {
        ...
        return AuditItem("로그 재구성 ↔ 레지스트리", "이벤트 로그", checked, "개 대조", violations)
    }

    // ------------------------------------------------------------------ 8·9. 부하 중 프로토콜 무결성

    /**
     * **부하 중에도 공식 스키마를 어긴 메시지가 하나도 없다** (설계원칙 2).
     *
    ...
    private fun schemaCompliance(): AuditItem {
        ...
        return AuditItem("공식 스키마 위반 0", "스키마 정본", checked, "건 메시지", violations)
    }
    ...
    private fun noUnexpectedCallErrors(): AuditItem {
        ...
        return AuditItem("CALLERROR 0", "적합성", checked, "건 프레임", violations)
    }
