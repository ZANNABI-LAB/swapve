...
| 지표 대시보드 · UI | 확정 결정 결정 #2 — **REST 조회까지가 범위**다 |
| 다국어(i18n) | 확정 결정 결정 #3 — 앱을 구현하지 않으므로 실질 요구가 없다 |
...
> 충전 트랜잭션 · 확정 결정 결정 #8: **수신·기록만.** 스마트차징·요금은 없다.
...
범위 밖이다(확정 결정 결정 #8). 그래서 `POST` 도 `PATCH` 도 없다.
...
3. **소비자가 없다.** 확정 결정 결정 #2 가 대시보드를 제외했다. Micrometer 의 값은
