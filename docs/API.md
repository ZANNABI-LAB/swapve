...
| 지표 대시보드 · UI | 확정된 범위 결정 — **REST 조회까지가 범위**다 |
| 다국어(i18n) | 확정된 범위 결정 — 앱을 구현하지 않으므로 실질 요구가 없다 |
| 페이징 · 목록 조회 (`GET /api/swaps`) | 소비자가 없다. 없는 소비자를 위해 엔드포인트를 미리 뚫지 않는다 (설계 원칙) |

닫힌 것은 닫힌 것으로 적는다. WebSocket 은 보안 프로파일 1 Basic, 이 REST API 는 별도 Basic,
`sim-console` 은 loopback 기본 바인딩이다. 남은 것은 mTLS·인증서 관리, 자격증명 회전,
...
**`requestId` 를 받지 않는다.** S02 의 발번 주체는 **CSMS** 다 (유즈케이스 표).
...
(재고 판정, **S02.FR.04**), CSMS 는 재고를 몰라도 된다. 이것이 공식 적합성 케이스
...
`reason_codes.csv`(디바이스 모델.1)로 해석한 결과다. 표에 없는 값이 와도 버리지 않는다 —
...
상태머신 의 상태머신 그대로다. **순서 불가지론**이라 입고 먼저(통상)든
...
`serialNumber` · `soC` · `soH` 를 **입고·출고 양쪽 다** 보존한다 (과금 여지).
...
| 축 | 값 |
|---|---|
| `byScenario` | 실패 시나리오 의 F1~F6 |
| `byReasonCode` | 부록 `reason_codes.csv` (디바이스 모델.1) 의 사유 |
...
`ledger` 만은 H2 에서 읽으므로 **재시작을 가로질러 남는다** (불변식 — 교환
...
> 충전 트랜잭션 · 확정된 범위 결정: **수신·기록만.** 스마트차징·요금은 없다.
...
범위 밖이다(확정된 범위 결정). 그래서 `POST` 도 `PATCH` 도 없다.
...
(설계 원칙) — 앱은 스테이션의 설정 변수를 만지지 않고, 운영 도구는 아직 없다.
...
1. **필요한 값이 전부 이미 있는 기록에서 파생 계산된다.** 성공률은 교환 보관소의 상태
   ...
   멱등 기록과 **이벤트 로그의 원문**에 있다. 이벤트 로그 이 이벤트 로그에 건
...
3. **소비자가 없다.** 확정된 범위 결정 가 대시보드를 제외했다. Micrometer 의 값은
...
(설계 문서 v3.1). 이 API 는 그 위에 REST 진입점을 얹은 것이고,
