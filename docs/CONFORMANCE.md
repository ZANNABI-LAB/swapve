...
성공 기준 의 성공 기준을 **실행 가능한 명령**으로 옮긴 표입니다.
...
> `NotifyReport`를 `requestId`별로 재조립합니다 (시뮬레이터 명세).
>
> 시험 대상(System under test)이 CS인 케이스(p.948–954)는 **시뮬레이터의 명세**로 씁니다
> (시뮬레이터 명세) — `BootedBatterySwapping` · `AuthorizedBatterySwapping` ·
