 ...
 * 이 분기를 그대로 구현하면 §4.6 양방향이 자동으로 검증된다.
 *
 ...
 * 그래도 **CSMS 의 상태머신은 이 값을 몰라야 한다** — 순서 불가지론.
 *   ⚠️ 이 변수는 **부록 CSV(`dm_components_vars.csv`)에 없고 Part 2 본문에만 있다.**
 * 부록은 릴리스와 별개로 갱신될 수 있으므로 본문을 따른다 (주의 3).
 */
...
/**
 * 슬롯 하나의 초기 구성. 슬롯 1개 = EVSE 1개다.
 *
 * [battery] 가 `null` 이면 빈 슬롯이고, 있으면 교환에 내줄 수 있는 배터리가 들어 있다.
 * 프로토콜의 `Available`/`Occupied` 로 바꾸는 일은 경계에서만 한다.
 *
 ...
 *
 * 이것이 `TC_S_103_CSMS` 의 실제 모습이다 (읽을 것 5): 입고 배터리는 tx `…-3`/`…-4`
 ...
 * 서로 다르다. 단일 교환만 지원하면 적합성 미달이다.
 *
 * ### requestId 는 스테이션이 발번한다
 *
 * S01 로컬 인가에서는 상관 번호를 스테이션이 정하고, 입고와 출고가 **같은 값을
 ...
 * SoC** (S04.FR.05). 철자의 끝이 대문자 `C` 인 것은 정본이 그렇기 때문이다 (주의 2).
...
data class StationSimConfig(
    ...
    init {
        ...
        // COMPLETED 불변식 — 들어온 배터리 수 = 나간 배터리 수.
