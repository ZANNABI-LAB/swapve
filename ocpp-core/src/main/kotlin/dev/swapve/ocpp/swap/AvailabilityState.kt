...
/**
 * ⚠️ **직관과 반대인 슬롯 가용성** (Part 2 §S Ch.2, PLAN §4.2).
 *
 ...
 * ### 이 반전 지식은 이 파일에만 있다
 *
 * PLAN §4.2 가 "버그 유발 1순위"로 지목한 것이 이 반전이다. 도메인은 `EMPTY`/`HOLDS_BATTERY`
 ...
 * swap-domain 에 두면 도메인에 프로토콜 어휘가 새어 들어가고 (PLAN §6 원칙 4), 그렇다고
 ...
 * `null` 이다 — 모른다는 사실을 `false` 로 뭉개지 않는다 (PLAN §11.0).
 */
