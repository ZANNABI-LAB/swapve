...
/**
 * 오간 OCPP 메시지 하나의 추가 전용(append-only) 기록 (PLAN §11.1).
 *
 * 필드는 PLAN §11.1 표 그대로다. **파생 상태가 이 로그에서 재구성 가능해야 한다는 것이
 * 유일한 규칙**이므로, 해석한 결과가 아니라 [payload] 에 **원문 그대로**를 남긴다.
 * 우리가 지금 이해하지 못하는 필드도 나중에 읽을 수 있어야 한다 (PLAN §11.0 — 정보를 버리지 않는다).
 *
 * 이 로그 하나가 과금·로밍·감사·수평확장 네 가지 확장을 동시에 연다 (PLAN §11.1 표).
 *
 * @param seq **스테이션 내** 증가 순번. 스테이션끼리는 비교 대상이 아니다 — 서로 다른 연결의
 *   메시지에 전역 순서를 매기는 것은 지킬 수 없는 약속이다 (PLAN §11.5 수평 확장).
 ...
 * 이벤트를 받아 적는 자리.
 *
 * PLAN §11.0 은 "구현체 1개짜리 인터페이스"를 금하지만, §11.1 이 **인터셉터 자리 하나**를
 * 명시적으로 요구한다. 지금은 인메모리 구현 하나뿐이고 DB 영속은 저장소 결정(§6) 뒤로 미룬다 —
 * 그 교체 지점이 여기다.
 *
 ...
 * 인메모리 이벤트 로그.
 *
 * **DB 영속은 이 마일스톤에서 하지 않는다** (PLAN §6 저장소 결정, §11.1 "테이블 1개 + 인터셉터 1개").
...
class InMemoryOcppEventLog : OcppEventSink {
    ...
    /** 한 스테이션의 기록을 `seq` 순서로. 재구성은 항상 이 단위로 한다 (PLAN §11.1). */
